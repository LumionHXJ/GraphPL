    ...
    for s_ind, s in enumerate(test_samples):
        ...
        with torch.no_grad():
            ...
            if exp.flags.impute_method == 'graph':
                l_dec = {'content': dict(),
                        'style': styles};
                for mid, mod in enumerate(modalities_names):
                    decoder_mask = exp.observed_mask.detach().clone()
                    decoder_mask[c_ind, mid] = 1
                    model.clear_gnn_cache()
        ...
        # os.makedirs(os.path.join(exp.flags.dir_cond_gen, 'impute'), exist_ok=True)
        # os.makedirs(os.path.join(exp.flags.dir_cond_gen, 'gt'), exist_ok=True)
        for m_i, m_k in enumerate(modalities_names):
            ...
            '''if observed_mask[m_i]==0:
                fn_out = os.path.join(os.path.join(exp.flags.dir_cond_gen, 'impute'), '%d_%s.png'%(s_ind, m_k));
                plot_out_grid = plot.create_fig(fn_out, pic_m, len(test_samples), save_figure=exp.flags.save_figure)
                fn_gt = os.path.join(os.path.join(exp.flags.dir_cond_gen, 'gt'), '%d_%s.png'%(s_ind, m_k));
                plot_gt_grid = plot.create_fig(fn_gt, pic_m_gt, len(test_samples), save_figure=exp.flags.save_figure)            
            '''
