...
DIM_IMG = [16, 32, 64, 128, 256]

class EncoderImgBinary(nn.Module):
    def __init__(self, class_dim, style_dim):
        super(EncoderImgBinary, self).__init__();
        self.feature_extractor = FeatureExtractorImg(1, DIM_IMG)
...
class DecoderImgBinary(nn.Module):
    def __init__(self, class_dim, style_dim):
        super(DecoderImgBinary, self).__init__()
        self.style_dim = style_dim
        self.feature_generator = nn.Linear(style_dim + class_dim, DIM_IMG[-1], bias=True);
        self.img_generator = DataGeneratorImg(1, DIM_IMG)
