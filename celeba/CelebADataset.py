...
class CelebaDataset(Dataset):
    ...
    def __init__(self, args, alphabet, partition=0, transform=None, pre_load=False):
        ...
        filename_text = os.path.join(self.dir_dataset_base, 'list_attr_text_256_celeba_lowercase.csv');
        ...
        self.seg_dir = os.path.join(self.dir_dataset_base, 'semantic_align_celeba')
        self.canny_dir = os.path.join(self.dir_dataset_base, 'canny_align_celeba')
        ...
        if pre_load:
            ...
            for i in range(self.y.shape[0]):
                img, canny, seg = self.load_img(i)
                self.imgs_list.append(img)
                self.canny_list.append(canny)
    ...
    def load_img(self, index):
        ...
        canny = Image.open(os.path.join(self.canny_dir, self.img_names[index]))
        seg = Image.open(os.path.join(self.seg_dir, self.img_names[index]))
        if self.transform is not None:
            img = self.transform(img)
            canny = self.transform(canny)
            seg = self.transform(seg)
        return img, canny, seg

    def __getitem__(self, index):
        if self.pre_load:
            img = self.imgs_list[index]
            canny = self.canny_list[index]
            seg = self.seg_list[index]
        else:
            img, canny, seg = self.load_img(index)
        ...
        sample = {'img': img, 'text': text_str, 'canny': canny, 'seg': seg}
        return sample, label
