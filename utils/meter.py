...
class SmoothedVector(SmoothedValue):
    @property
    def median(self):
        d = torch.tensor(list(self.averaged_value_deque))
        return d.median(dim=0).item()

    @property
    def avg(self):
        d = torch.tensor(list(self.deque))
        s = torch.tensor(list(self.batch_sizes))
        return d.sum(dim=0).item() / s.sum().item()

class MeterVector(Meter):
    def __init__(self, *args, **kwargs):
        super().__init__(args, kwargs)
        self.meters = defaultdict(SmoothedVector)
