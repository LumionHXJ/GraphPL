...
def calc_elbo(exp, modality, recs, klds):
    ...
    if modality == 'joint':
        ...
        kld_style = 0
        rec_error = r_weights[modality] * recs[modality];
...
def avg_list(scalar_list, weights=None):
    if weights is None:
        weights = [1.0]*(len(scalar_list))
    weights = np.array(weights)/np.sum(weights)
    return np.sum(np.array(scalar_list) * weights)
...
def get_longest_subsets(modalities):
    """
    powerset([1,2,3]) --> () (1,) (2,) (3,) (1,2) (1,3) (2,3) (1,2,3)
    """
    xs = list(modalities) # list(self.modalities.keys())
