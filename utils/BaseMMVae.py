...
from utils.BaseGCN import compute_class_dim


class BaseMMVae(ABC, nn.Module):
    def __init__(self, flags, modalities, subsets, mod_dimensions):
    ...
    def poe_fusion(self, mus, logvars, weights=None):
        if (self.flags.impute_method == 'poe' or mus.shape[0] == 
            ...
            # p(z|x) = p(z) \prod_i q(z|x_i)
            num_samples = mus[0].shape[0];
            mus = torch.cat((mus, torch.zeros(1, num_samples,
                             self.class_dim).to(self.flags.device)),
                            dim=0);
    ...
    def forward(self, input_batch, subset_used, *args, **kwargs):
        ...
        for m, m_key in enumerate(self.modalities.keys()):
            if m_key in input_batch.keys():
                ...
                if self.styles[m_key] > 0:
                    ...
                    m_s_embeddings = None;
                c_embeddings = class_embeddings.reshape(-1, self.mod_dimensions[m_key], 
                                                        self.dim_multiples[m_key]).mean(dim=2)
                if hasattr(self.decoders[m_key], 'teacher_forcing'):
                    m_rec = self.lhoods[m_key](*self.decoders[m_key](m_s_embeddings, c_embeddings, input_batch[m_key]))
                else:
                    m_rec = self.lhoods[m_key](*self.decoders[m_key](m_s_embeddings, c_embeddings))
                ...
                if self.flags.impute_method == 'collaborate' and self.flags.k_single!=0:
                    ...
                    if hasattr(self.decoders[m_key], 'teacher_forcing'):                
                        m_rec_single = self.lhoods[m_key](*self.decoders_single[m_key](m_s_embeddings_single, m_class_embeddings, input_batch[m_key]))
                    else:
    ...
    def inference(self, input_batch, subset_used, num_samples=None):
        ...
        for k, s_key in enumerate(self.subsets.keys()):
            if s_key != '':
                ...
                if self.flags.dataset=='eicu' and (not self.fusion_condition(mods, input_batch, s_key, subset_used)) and \
                        (len(mods)!=1) and (s_key != '_'.join(sorted(list(input_batch.keys())))):
                    continue
        ...
        if self.flags.impute_method == 'collaborate':
            ...
            latents['mus_single'] = mus_single # M, B, L: m0, m2, m4
            latents['logvars_single'] = logvars_single;
        joint_mu, joint_logvar = self.moe_fusion(mus, logvars, weights);
        latents['mus'] = mus # satisfied fusion cond: such as m0_m2, m2, m4
        latents['logvars'] = logvars;        
        latents['weights'] = weights

        latents['joint'] = [joint_mu, joint_logvar]; # B, L
        latents['subsets'] = distr_subsets # hold all subsets
        latents['subsets_in_fusion'] = subsets_in_fusion # subsets satisfied fusion cond
        return latents;
    ...
    def generate_sufficient_statistics_from_latents(self, latents, imputed_mods=None, target_sequence=None):
        ...
        for m, m_key in enumerate(imputed_mods):
            if self.styles[m_key] > 0:
                ...
                s = None
            content_rep = content.reshape(-1, self.mod_dimensions[m_key], 
                                              self.dim_multiples[m_key]).mean(dim=2)
            if target_sequence is not None and hasattr(self.decoders[m_key], 'teacher_forcing'):
                cg = self.lhoods[m_key](*self.decoders[m_key](s, content_rep, target_sequence[m_key]))
            else:
                cg = self.lhoods[m_key](*self.decoders[m_key](s, content_rep))
            suff_stats[m_key] = cg;
        return suff_stats
    ...
    def generate_from_distribution(self, distribution):
        ...
        suff_stats = self.generate_sufficient_statistics_from_latents(latents);
        cond_gen = dict();
        for m, m_key in enumerate(suff_stats.keys()):
            cond_gen_m = suff_stats[m_key].mean;
            cond_gen[m_key] = cond_gen_m;
        return cond_gen;
