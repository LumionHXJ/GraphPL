...
parser.add_argument('--port', type=str, default='19824')

# TRAINING
parser.add_argument('--ablation_level', type=int, default=0, help="1 for drop 33%, 2 for drop 67%, 3 for pure average")
...
parser.add_argument('--warmup_epoch', type=int, default=0, help="only reconstruct in warmup epoch")
parser.add_argument('--end_epoch', type=int, default=1000, help="flag to indicate the final epoch of training")
parser.add_argument('--communication_freq', type=int, default=1, help="continue ... local round then communicate with server")
parser.add_argument('--validation_freq', type=int, default=5)
parser.add_argument('--test_only', default=False, action="store_true", help="")
parser.add_argument('--teacher_forcing', type=float, default=0.2, help="Teacher forcing rate for auto_regressive manner.")
parser.add_argument('--kl_annealing', type=int, default=0, help="number of kl annealing steps; 0 if no annealing should be done")

# DATA DEPENDENT
parser.add_argument('--dataset', type=str, default='', help="[polymnist, mnistsvhntext]")
...
parser.add_argument('--use_modality_weight', default=False, action="store_true", help="")
parser.add_argument('--use_modality_embedding', default=False, action="store_true", help="")
parser.add_argument('--observed_mask', type=str, help="load specific mask file")
...
# data
parser.add_argument('--dir_data', type=str, default='', help="directory where data is stored")
# experiments
parser.add_argument('--dir_experiment', type=str, default='/tmp/exp', help="directory to save generated samples in")

# EVALUATION
parser.add_argument('--noisy_impute', default=False, action="store_true",
                    help="testing robustness while facing noisy modalities")
...
parser.add_argument('--calc_auprc', default=False, action="store_true",
                    help="flag to indicate evaluation of calc_auprc")
...
# multimodal
parser.add_argument('--impute_method', type=str, default='graph', help='choose method for training the model')
# with graph
parser.add_argument('--fusion_method', type=str, default='zero', help='choose fusion method for the model, poe/moe/zero')
...
parser.add_argument('--k_single', type=float, default=1.0, help="using single decoder if k_single > 0")
