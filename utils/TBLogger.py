...
def dist_writer_process(tb_logger, exp, start_epoch):
    log_file = exp.flags.log_pth
    client_pattern = r'client\s(\d+):'
    train_test_pattern = r'\b(Train|Test)\b'
    with open(log_file) as f:
        # find start position
        while True:
            l = f.readline()
            # due to barrier: epochs are in order
            if f"------Epoch {start_epoch}/{exp.flags.end_epoch}---------" in l:
                break
        # start to write log
        for ll in f.readlines():
            if f'epoch {start_epoch}' in ll:
                # mode: train or test
                train_test_match = re.search(train_test_pattern, ll)
                mode = train_test_match.group(1) if train_test_match else None 

                log_content = 'loss:' + ll.split('loss:', 1)[1]
                log_items = [item.strip() for item in log_content.split(";") if item.strip()]
                logger_dict = {}
                for item in log_items:
                    key, value = item.split(": ")
                    logger_dict[key.strip()] = float(value)

                # if summary
                if 'all' in ll:
                    tb_logger.write_dict({f'{mode}/{k}_all':logger_dict[k] for k in logger_dict.keys()}, 
                                        step=start_epoch)
                else:
                    # fetch cliend id  
                    continue                  
                    client_match = re.search(client_pattern, ll)
                    client_id = client_match.group(1) if client_match else None
                    tb_logger.write_dict({f'{mode}/{k}_client{client_id}':logger_dict[k] for k in logger_dict.keys()}, 
                                            step=start_epoch)
            else:
                assert f'Epoch {start_epoch + 1}' in ll
                start_epoch += 1
