...
class ClfText(nn.Module):
    def __init__(self, flags):
        ...
        self.flags = flags
        self.conv1 = nn.Conv1d(flags.num_features, 2 * 64, kernel_size=1);
        self.resblock_1 = make_res_block_encoder(2 * 64, 3 * 64, kernelsize=4, stride=2, padding=1,
                                                 dilation=1);
        self.resblock_4 = make_res_block_encoder(3 * 64, 2 * 64, kernelsize=4, stride=2, padding=0,
                                                 dilation=1);
        self.dropout = nn.Dropout(p=0.5, inplace=False);
        self.linear = nn.Linear(in_features=2*64, out_features=10, bias=True) # 10 is the number of classes
