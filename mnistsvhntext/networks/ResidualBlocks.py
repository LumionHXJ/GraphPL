...
class ResidualBlock2dConv(nn.Module):
    def __init__(self, channels_in, channels_out, kernelsize, stride, padding):
        ...
        self.conv1 = nn.Conv2d(channels_in, channels_in, kernel_size=3, stride=1, padding=1, bias=False)
...
class ResidualBlock2dTransposeConv(nn.Module):
    def __init__(self, channels_in, channels_out, kernelsize, stride, padding):
        ...
        self.conv1 = nn.ConvTranspose2d(channels_in, channels_in, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(channels_in)
        self.relu = nn.ReLU(inplace=True)
        self.bn2 = nn.BatchNorm2d(channels_out)
        self.conv2 = nn.ConvTranspose2d(channels_in, channels_out, kernel_size=kernelsize, stride=stride, padding=padding, bias=False, output_padding=stride-1)
        self.upsample = nn.Sequential()
        if stride != 1 or channels_in != channels_out:
            self.upsample = nn.Sequential(
                nn.ConvTranspose2d(channels_in, channels_out, kernel_size=1, stride=stride, output_padding=stride-1, bias=False),
                nn.BatchNorm2d(channels_out)
            )
        self.alpha = nn.Parameter(torch.zeros(1))
