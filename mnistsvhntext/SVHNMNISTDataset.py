...
def match_svhn_to_mnist(ls, idxs, lm, idxm, dm=2):
    """
    l*: sorted labels
    idx*: indices of sorted labels in original list
    s: svhn, m: mnist
    match results: almost class-balanced
    """
    _idxs, _idxm = [], []
    for l in ls.unique():  # assuming both have same idxs
        l_idxs, l_idxm = idxs[ls == l], idxm[lm == l]
        n = l_idxm.size(0) * dm
        for _ in range(dm):
            _idxm.append(l_idxm[torch.randperm(l_idxm.size(0))])
        while True:
            if n <= l_idxs.size(0):
                pick = torch.randperm(l_idxs.size(0))[:n]
                _idxs.append(l_idxs[pick])
                break
            else:
                _idxs.append(l_idxs[torch.randperm(l_idxs.size(0))])
                n -= l_idxs.size(0)
    return torch.cat(_idxs), torch.cat(_idxm)
...
class SVHNMNIST(VisionDataset):
    ...
    def __init__(self, flags,  alphabet, train=True, transform=None, target_transform=None):
        ...
        self.svhn_idx, self.mnist_idx = match_svhn_to_mnist(svhn_l, svhn_li,
                                                            mnist_l, mnist_li,
