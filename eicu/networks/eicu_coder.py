...
class eicu_encoder(nn.Module):
    def __init__(self, flags):
        ...
        self.class_mu = nn.Linear(2*hidden_dim, flags.dim)
        self.class_logvar = nn.Linear(2*hidden_dim, flags.dim)
    ...
    def forward(self, x):
        ...
        bilstm_out = bilstm_out.permute(1, 2, 0) # [N, 2*hidden_dim, L]
        bilstm_out = torch.tanh(bilstm_out)
        bilstm_out = F.max_pool1d(bilstm_out, kernel_size=int(bilstm_out.size(2))).squeeze(2)
...
class eicu_decoder(nn.Module):
    def __init__(self, flags):
        ...
        self.linear = nn.Sequential(
            nn.Linear(flags.dim, 2*hidden_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(2*hidden_dim, 2*hidden_dim),
            nn.ReLU()
